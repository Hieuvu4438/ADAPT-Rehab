...
class AUData:
    ...
    au7_approx: float = 0.0  # Lid Tightener (ước lượng từ EAR)
    au9: float = 0.0    # Nose Wrinkler
    au10_approx: float = 0.0  # Upper Lip Raiser (ước lượng từ lip geometry)
...
class EARCalculator:
    ...
    def compute_au7_intensity(self, ear: float) -> float:
        """
        AU7 Lid Tightener — mí căng khi nheo mắt.
        Active khi EAR giảm 15-50% so với baseline nhưng chưa đến ngưỡng nhắm.
        """
        ear_baseline = self._ear_open_baseline  # ~0.30

        # Ngưỡng AU7: EAR giảm nhưng chưa đến threshold nhắm (0.20)
        au7_upper = ear_baseline * 0.85  # ~0.255 — bắt đầu nheo
        au7_lower = self.ear_threshold  # ~0.20  — sắp nhắm

        if ear > au7_upper:
            return 0.0  # Mắt mở bình thường
        elif ear <= au7_lower:
            return 5.0  # Mắt gần nhắm hoàn toàn → AU7 max
        else:
            # Linear từ 0 đến 5 trong vùng [au7_lower, au7_upper]
            ratio = (au7_upper - ear) / (au7_upper - au7_lower)
...
class PSPICalculator:
    ...
    def compute(self, au_data: AUData) -> float:
        ...
        pspi = (
                au_data.au4 +
                max(au_data.au6, au_data.au7_approx) +  # đúng công thức gốc
                max(au_data.au9, au_data.au10_approx) +
                au_data.au43_approx
        )
        return pspi
...
class FacialStateDetector:
    ...
    def process_frame(self, au_raw: Dict[str, float],
        ...
        if face_landmarks is not None:
            ...
            au_data.au7_approx = self.ear_calculator.compute_au7_intensity(ear)  # thêm dòng này
